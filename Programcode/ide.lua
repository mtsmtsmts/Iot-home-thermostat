function editor(conn) -- feel free to disable the shiny Ajax.org Cloud Editor
    ...
    if url ~= "" and vars == "" then
        ...
        sen = sen .. "<html><head><title>NodeMCU IDE Thermostat</title><meta name=\"viewport\" content=\"width=device-width,initial-scale=1.0\"><meta http-equiv=\"Expires\" content=\"-1\" />"
        sen = sen .. "<style>a:link{color:white;} a:visited{color:white;} a:hover{color:yellow;} a:active{color:green;}</style></head>"
        sen = sen .. "<body style=\"background-color:#333333;color:#dddddd\"><h1><a href='/'>NodeMCU IDE Thermostat</a></h1>"
    if vars == "edit" then
        if AceEnabled then
            local mode = 'ace/mode/'
